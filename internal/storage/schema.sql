...
CREATE TABLE IF NOT EXISTS events (
  ...
  UNIQUE INDEX uq_replaceable (pubkey ASC, kind ASC) WHERE kind = 0:::INT8 OR kind = 3:::INT8 OR kind = 41:::INT8 OR (kind >= 10000:::INT8 AND kind < 20000:::INT8),
--   UNIQUE INDEX uq_addressable (pubkey ASC, kind ASC, (tags->>'d':::STRING) ASC) WHERE (kind BETWEEN 30000:::INT8 AND 39999:::INT8) AND (tags ? 'd':::STRING),
  UNIQUE INDEX uq_addressable (pubkey ASC, kind ASC, (jsonb_path_query_first(tags, '$[*]?(@[0] == "d")[1]'::JSONPATH,'{}'::JSONB,true)::STRING) ASC) WHERE (kind >= 30000::INT8 AND kind < 40000::INT8) AND jsonb_path_exists(tags, '$[*]?(@[0] == "d")'::JSONPATH),
